...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_URL}/avaliacao/dados-fisicos`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Falha ao criar dados físicos: ${errorData}`);
    }

    const result = await response.json();
    console.log('Resposta do backend (dados físicos):', result);
